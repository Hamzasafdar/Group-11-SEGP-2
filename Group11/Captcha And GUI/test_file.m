function inputMatrix = test_file(image)

% directory = dir('F:\matlab\Data Set\*.png');         %read all images in directory
% directory = dir('C:\Users\Hamza Safdar\Dropbox\ocr-segp17-18\Hamza\Data Set\EnglishFnt\English\Fnt\Sample011\*.png'); 
noOfFiles = length(image);    %get file count
inputMatrix = ones(noOfFiles,15);
for loop = 1 : noOfFiles
%    tempName = directory(loop).name;
%    imageData = imread(tempName);
   [width,height,blackPixels,bbx,bby,bbh,bbw,s_length,feature,startingXav,startingYav,endingXav,endingYav,thetaav,rhoav] = feature_extractor(image{loop});
